...
{
    //add 2017/12/20 - by SYRIS_ControllerUtility
    public class SyrisAES  // Advanced Encryption Standard
    {
        //static public String StrAESKey = "36649AF26DDE0C3A0F1E2D3C4B5A6978";
        static public byte[] AESKey = { 0x36, 0x64, 0x9A, 0xF2, 0x6D, 0xDE, 0x0C, 0x3A, 0x0F, 0x1E, 0x2D, 0x3C, 0x4B, 0x5A, 0x69, 0x78 };//Syris 預設值
        ...
        private int round;
        public SyrisAES(KeySize keySize)
        {
            SetNbNkNr(keySize);

            this.key = new byte[this.Nk * 4];  // 16, 24, 32 bytes
            AESKey.CopyTo(this.key, 0);
        ...
        public SyrisAES(KeySize keySize, byte[] keyBytes)
        {
            SetNbNkNr(keySize);
        ...
        //---------------------------------------------------------------------------
        public void AES(byte[] input, byte[] output)
        {
            int i;
            int intShift = 0;
            for (i = 0; i < input.Length; i += 16)
            {
                ...
                Array.Copy(input, intShift, inputbuf, 0, 16);
                byte[] outputbuf = new byte[16];
                Cipher(inputbuf, outputbuf);
                Array.Copy(outputbuf, 0, output, intShift, 16);
                intShift += 16;
            }
        }
        ...
        // blocklen : multiple of 16
        //---------------------------------------------------------------------------
        public void UnAES(byte[] input, byte[] output)
        {
            int i;
            int intShift = 0;
            for (i = 0; i < input.Length; i += 16)
